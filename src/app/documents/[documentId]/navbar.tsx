...
export const Navbar = ({ data }: NavbarProps) => {
    ...
    const onDownload = (blob: Blob, filename: string) => {
        // Blob 对象是 JavaScript 中用于表示 原始数据 的类，它可以包含任何类型的二进制数据，
        // 例如文本文件、图像文件、音频文件、视频文件等。Blob 对象常用于处理文件和大数据传输，比如上传和下载文件。
        const url = URL.createObjectURL(blob); // 创建一个指向 Blob 对象内容的临时 URL
        const a = document.createElement("a");
        a.href = url; // 创建一个 <a> 标签，并将该 URL 设置为其 href 属性
        a.download = filename; // 设置下载文件的名称
        a.click(); // 模拟点击 <a> 标签，从而触发浏览器下载该文件
    };
